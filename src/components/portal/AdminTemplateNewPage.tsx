...
export function AdminTemplateNewPage() {
  const router = useRouter();
  const { createTemplate, refresh } = useApp();

  const [form, setForm] = useState<TemplateForm>(() => catalogToForm("Launch"));
  ...
  function applySelectedWorkflow() {
    setForm(catalogToForm(form.packageType));
    setNotice("Default workflow applied. You can adjust it before saving.");
    setError("");
  }
  ...
  function changePackage(packageType: PackageType) {
    const item = getPackageCatalogItem(packageType);

    setForm((current) => ({
      ...current,
      packageType,
      name: current.name.trim() ? current.name : item.title,
      description: current.description.trim() ? current.description : item.description,
      phases: current.phases.length ? current.phases : catalogToForm(packageType).phases,
    }));
  }
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-white/82 sm:text-[15px]">
                  Build a reusable package workflow with phases and deliverables that can be used across client, staff and admin project creation.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  <Field label="Package / Suite">
                    <Select
                      ...
                      disabled={loading}
                      onChange={(event) =>
                        changePackage(event.target.value as PackageType)
                      }
                  ...
                  <Field label="Template name">
                    <Input
                      ...
                      placeholder="e.g. Launch Suite Workflow"
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm leading-6 text-slate-600">
                    Define the workflow clients and delivery teams will follow after project approval.
                  </p>
                </div>
